...
`npm test` — 34 suites, against the real exported functions with synthetic events. No sockets,
...
admission · admission return-lane lifecycle · message rendering · deployed-build verification · routing-policy snapshot · return lane · return-lane scan ·
...
CI runs them on push and PR. **If a run reports fewer than 34, the branch is on a stale base.**
