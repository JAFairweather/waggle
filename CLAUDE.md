...
`npm test` — 40 suites, against the real exported functions with synthetic events. No sockets,
no production state, no writes outside a temp dir.

boot · suite roster · off-box policy protocol · policy journal · policy-owned Buzz artifacts · Nostr remote signer · read resilience · egress catalogue · egress ban · durable dedup store · relay fan-out · quarantine gating · deletion propagation · sealed-lane rate caps · grant
...
CI runs them on push and PR. **If a run reports fewer than 40, the branch is on a stale base.**
