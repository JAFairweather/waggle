...
`npm test` runs 114 suites against real exported functions with synthetic events—no production state and no network. The join-custody suite drives `tools/join.mjs` over a loopback TLS socket on an ephemeral port, since that tool's transport is wss-only by design. The suite is designed to prove what the bridge refuses as carefully as what it delivers:
