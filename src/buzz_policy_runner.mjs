...
export function loadBuzzPolicyConfig(path, env = process.env, { requireRecovery = true } = {}) {
  ...
  const sealedKeys = [...configKeys, 'recipient_routes']
  const actualKeys = Object.keys(config).sort().join(',')
  const shapes = [legacyKeys, configKeys, sealedKeys].flatMap(keys => [keys, [...keys, 'recovery_secret_file']])
  ...
  const loaded = { ...config, inbox_channel: config.inbox_channel || null,
    ...
    recipient_routes: Object.freeze(Object.fromEntries(Object.entries(recipientRoutes).map(([key, value]) => [key, Object.freeze({ ...value })]))), artifactPolicy }
