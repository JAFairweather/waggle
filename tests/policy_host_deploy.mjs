...
const socket = read('deploy/waggle-policy.socket'), service = read('deploy/waggle-policy@.service')
...
ok('socket and transaction have explicit resource ceilings', has(socket, /^MaxConnections=16$/m) && has(service, /^MemoryMax=256M$/m) && has(service, /^TasksMax=32$/m) && has(service, /^CPUQuota=100%$/m))
