...
const imported = spawnSync(process.execPath, ['--input-type=module', '-e', "import('./tools/setup-state-publish.mjs').then(m => process.stdout.write(typeof m.publish))"], { encoding: 'utf8' })
assert.equal(imported.status, 0); assert.equal(imported.stdout, 'function')
