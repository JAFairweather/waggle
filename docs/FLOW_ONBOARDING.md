...
| 8 | **Wake** — the agent's watcher runs a hook | `tools/agent-inbox.mjs` (`--trust`, `--on-message`, `--spool`) | a spool record with a wake verdict | the hook's own output file grows | **without `--trust` every message is recorded as data**, `mayAct` is false, and the lane looks completely healthy (`agent-inbox.mjs:301`). The tool refuses `--on-message` with an empty trust list for exactly this reason — on the keyed path. `--notify-only` is the keyless watcher for a host we do not control: it holds no signer, refuses `--trust` outright (the sender is inside the seal), and its records carry `trust_evaluated: false` so the wake is never mistaken for authority |
